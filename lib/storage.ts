...
export const isPurchased = (): boolean => {
  console.warn(
    "isPurchased() устарела, используйте isPurchaseActive(courseId)",
  );
...
export const setPurchased = (value: boolean): void => {
  console.warn(
    "setPurchased() устарела, используйте setPurchase(courseId, purchaseData)",
  );
  if (value) {
...
export const addToProgress = (
  lessonId: string,
  courseId: string = "brow-master-pro",
): void => {
